...
public class ManagementController implements Observer<ResEntityChangeEvent> {
    ...
    void initialize() {
        ...
        allTrips.setRowFactory(tv -> {
            TableRow<Trip> row = new TableRow<>();
            row.itemProperty().addListener((observable, oldItem, newItem) -> {
                // Check the value of the specific cell that you're interested in
                if (newItem != null && newItem.getAvailableSpots() ==0 ) {
                    // Set the text color of the row to red
